...
export type UiAnswer = {
  ...
  question: string;      // we don't get a question back; show category as heading fallback
  answer: string;
  categories: string[];
  stageFit: string[];
  helpfulFor: number;
...
function mapAnswer(a: BackendAnswer): UiAnswer {
  ...
  return {
    id: a.matched_id,
    question: a.answer_summary.slice(0, 60) + (a.answer_summary.length > 60 ? "…" : ""),
    ...
    helpfulnessPct: Math.round(a.match_score * 100),
