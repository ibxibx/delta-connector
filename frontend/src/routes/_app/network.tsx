...
import type { PersonalConnection } from "@/components/network-globe-personal";
import { NetworkFilterBar, type Lens } from "@/components/network-filter-bar";
import NetworkGraphPersonal from "@/components/network-graph-personal";

const NetworkGlobePersonal = lazy(() => import("@/components/network-globe-personal"));

const searchSchema = z.object({
  lens: fallback(z.enum(["globe", "graph", "list"]), "globe").default("globe"),
  topic: fallback(z.string(), "All").default("All"),
  q: fallback(z.string(), "").default(""),
  second: fallback(z.boolean(), true).default(true),
  active: fallback(z.boolean(), false).default(false),
  minTrust: fallback(z.number().min(0).max(100), 0).default(0),
...
export const Route = createFileRoute("/_app/network")({
  ...
  head: () => ({
    meta: [
      { title: "Network — Delta Connector" },
      { name: "description", content: "Your trusted connections across the world — globe, graph, and list, one workspace." },
    ],
  }),
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm">
            Three lenses on the same trust paths. Switch to see <em>where</em>, <em>how</em>, or <em>who</em>.
          </p>
        </div>
...
function LensLoading() {
  return <div className="text-muted-foreground/60 text-sm animate-pulse">Loading…</div>;
}
